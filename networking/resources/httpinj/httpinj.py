...
def inject_packet(flagged_packet):
    ...
    http_response = payload_mod.payload.http_payload()

    # Spin up a new packet
    to_inject = Ether() / IP() / TCP() / http_response
...
def main(argv):
    ...
    def print_help():
        print("""
        (python3.11 |./)httpinj.py [-i interface] [-r regexp] [-p payload] [-d dst_eth] [-j injection_interface] expression
    ...
    try:
        opts, args = getopt.getopt(argv[1:], 'i:r:p:d:j:h',
                                   ['interface=', 'regexp=', 'payloadname=', 'dst_eth=', 'injection_interface=',
                                    'help'])
